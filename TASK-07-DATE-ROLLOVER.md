...
A seat west of UTC writes a handoff just before local midnight. The name uses the local date `20260910`. The receipt is minted after 00:00 UTC the next calendar day, so its name uses `20260911`. The receipt’s `re` field still names the original handoff.
...
Naive join (`handoff.name[:8] == receipt.name[:8]`) returns no pair. The work looks unmatched even though `re` is exact.
...
Do not parse, increment, or “fix up” dates. Date prefixes are display, not keys.
...
Same-day names hide the bug. +1 day is the UTC rollover that drops a valid receipt. −1 day is the opposite skew (receipt named with yesterday relative to the handoff’s local date) and is also dropped by the prefix predicate.
